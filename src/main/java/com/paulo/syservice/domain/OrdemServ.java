...
public class OrdemServ {
	...
	public OrdemServ() {
		...
		this.setPrioridade(getPrioridade().BAIXA);
		this.setStatus(getStatus().ABERTO);

	}

	public OrdemServ(Integer id, Prioridade prioridade, String observacoes, Status status, Tecnico tecnico,
			Cliente cliente) {
		super();
		this.id = id;
		this.setDataAbertura(LocalDateTime.now());
